...
const apiURL = process.env.NODE_ENV === 'production'
    ? process.env.URL_PATH || 'https://aquarius.tubaron.com.br'
    : 'http://localhost:3000';
...
async function loadTeams() {
    try {
        ...
        console.error("Erro no carregamento das equipes:", error.message);
        if (apiURL.includes('localhost')) {
            alert("Servidor local não disponível. Certifique-se de que está rodando.");
        } else {
            alert("Erro ao acessar o servidor de produção.");
        }
    }
}
...
async function eliminateTeam(teamId) {
    ...
    try {
        const response = await fetch(`${apiURL}/teams/${teamId}/eliminate`, {
            method: "PATCH",
            headers: {
